...
class SolarReturnChartController implements ReportControllerInterface {
	...
	public function process( $options = [] ) {
		...
		$solarReturnYear = isset( $_POST['solar_return_year'] ) ? sanitize_text_field( wp_unslash( (int) $_POST['solar_return_year'] ) ) : '';
		...
		if (in_array('chart', $filter)) {
			...
			$chart = $method->process($location, $datetime, $progressionLocation, $solarReturnYear, $houseSystem, $orb, $birthTimeUnknown, $rectificationChart, $aspectFilter);
		}
		if (in_array('aspect-chart', $filter)) {
			$method = new SolarReturnAspectChart( $client );
			$aspectChart = $method->process($location, $datetime, $progressionLocation, $solarReturnYear, $houseSystem, $orb, $birthTimeUnknown, $rectificationChart, $aspectFilter);
		}
		if (in_array('planet-positions', $filter)) {
			$method = new SolarReturnPlanetPositions( $client );
			$result = $method->process($location, $datetime, $progressionLocation, $solarReturnYear, $houseSystem, $orb, $birthTimeUnknown, $rectificationChart);
		}

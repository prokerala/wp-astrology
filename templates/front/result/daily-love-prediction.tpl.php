...
<div class="pk-astrology-prediction-row pk-astrology-theme-<?php echo esc_attr( $options['theme'] ); ?>">
	<?php foreach ( $result as $sign_one => $data ) : ?>
		<?php foreach ( $data as $sign_two => $info ) : ?>
			<h3><?php echo $info['combination'];  // phpcs:ignore WordPress.Security.EscapeOutput ?></h3>
			<p><?php echo $info['prediction'];  // phpcs:ignore WordPress.Security.EscapeOutput ?></p>
		<?php endforeach; ?>
	<?php endforeach; ?>
</div>
